...
[[300, 565, 120, 500, 44, '#3d8a3c'], [300, 565, 480, 505, 44, '#478f43'],
 [300, 565, 210, 440, 36, '#4e9a45'], [300, 565, 395, 445, 36, '#3a8039'],
 [300, 565, 60, 545, 40, '#2f7433'],  [300, 565, 540, 550, 40, '#337a35']
].forEach(l => leaf(...l));

/* ============ armado del ramo ============ */
const FLOWERS = [
  { build: sunflower, x: 300, y: 175, s: 1.0,  r: 0,   sw: 1.2, dur: 8, stem: 'M300 610 C310 480 290 330 300 175' },
  { build: rose,      x: 170, y: 300, s: 1.0,  r: -17, sw: 2.5, dur: 6, stem: 'M300 610 C280 520 205 410 170 300' },
  { build: narciso,   x: 430, y: 290, s: 0.9,  r: 10,  sw: 3,   dur: 7, stem: 'M300 610 C320 520 400 400 430 290' },
  { build: dandelion, x: 95,  y: 430, s: 0.62, r: -8,  sw: 3,   dur: 9, stem: 'M300 610 C250 570 150 510 95 430' },
  { build: lotus,     x: 490, y: 475, s: 0.46, r: 28,  sw: 2,   dur: 7, stem: 'M300 610 C350 585 440 550 490 475' },
  { build: tulip,     x: 300, y: 405, s: 0.55, r: 0,   sw: 2,   dur: 6, stem: 'M300 610 C300 540 300 470 300 405' }
];
const stems = document.getElementById('stems');
const heads = document.getElementById('heads');
FLOWERS.forEach((f, i) => {
  const t = 0.4 + i * 0.3; // primero crece el tallo, luego florece la cabeza
  el('path', { class: 'stem', d: f.stem, pathLength: 1, style: `--d:${t.toFixed(2)}s` }, stems);
  const outer = el('g', { transform: `translate(${f.x} ${f.y}) rotate(${f.r}) scale(${f.s})` }, heads);
...
function play() {
  ...
  timer = setTimeout(() => svg.classList.add('open'), reduce ? 0 : 2600); // el tulipán y el loto se abren
}
